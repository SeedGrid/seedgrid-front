...
function buildRhfCode(code: string, baseName = "telefone", defaultValues?: string) {
  const matches = code.match(/<SgInputPhone\b/g) ?? [];
  const count = Math.max(matches.length, 1);
  const names = Array.from({ length: count }, (_, index) => `${baseName}${count > 1 ? index + 1 : ""}`);
  let cursor = 0;
  const withControl = code.replace(/<SgInputPhone\b/g, () => {

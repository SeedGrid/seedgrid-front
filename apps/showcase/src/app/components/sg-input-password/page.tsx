...
function buildRhfCode(code: string, baseName = "senha", defaultValues?: string) {
  const matches = code.match(/<SgInputPassword\b/g) ?? [];
  const count = Math.max(matches.length, 1);
  const names = Array.from({ length: count }, (_, index) => `${baseName}${count > 1 ? index + 1 : ""}`);
  let cursor = 0;
  const withControl = code.replace(/<SgInputPassword\b/g, () => {
    const name = names[cursor] ?? baseName;
    cursor += 1;
    return `<SgInputPassword\n  name="${name}"\n  control={control}`;
  });
